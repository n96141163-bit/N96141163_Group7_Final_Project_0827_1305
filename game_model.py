...
class GameModel:
    def __init__(self, user): 
        ...
        self.mileage_group = pygame.sprite.Group()           #新增4項
    ...
    def enemy_generator(self):
        ...
        if now - self.enemy_generator_time > self.enemy_generator_delay/(Glo.SPEED_RATE+0.05):
            ...
            self.enemy_group.add(enemy)             #enemy1和2加入enemy group
            enemy_type = "mileage"
            ...
            if self.money > 1000 and random.randint(0,2) > 1:
                #GameView.draw_text(surface, f"  花 費 時 間  ", 20, WHITE, False, 325, SCREEN_HEIGHT/2)
                self.siren.play()
                enemy = EnemyFactory.create("police", random.choice([i for i in range(150, SCREEN_WIDTH-150) if not 300 <= i <= 350]), 0, self.enemy_bullet_group)
    ...
    def check_for_collisions(self):
        ...
        bosses_hit_player = pygame.sprite.spritecollide(self.spaceship_group.sprite, self.boss_group, False)
        for boss in bosses_hit_player:
            #boss.hp -= 10
            pass
